...
const FrameworkGrid = () => {
  const renderedFrameworks = useMemo(
    () => frameworks.map(({ Icon, title, color, bg, border, outline, description }) => (
          <div data-aos="zoom-in" key={title} className={`group glass-effect relative rounded-2xl p-6 flex flex-col items-start hover:bg-slate-500/10 border-b-0 border-l-2 hover:border-l-[3px] ${border} outline outline-offset-4 outline-slate-500/15 ${outline}`}>
